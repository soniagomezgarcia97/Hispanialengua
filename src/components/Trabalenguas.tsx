...
const TrabalenguasComp = ({ titulo, texto, urlImagen }: PropsType) => {
    ...
    return (
        <>
            <button className="bg-no-repeat bg-cover bg-center w-full h-full flex flex-row justify-center items-end pb-[10%] hover:scale-110" style={{ backgroundImage: `url(${urlImagen})` }} onClick={() => { setAbrirTrabalenguas(true) }} title="abrir trabalenguas">
            ...
            {abrirTrabalenguas && (
                <div className="fixed z-10 top-1/2 left-1/2 w-[90%] -translate-x-1/2 -translate-y-1/2 max-h-[100vh] overflow-y-auto flex flex-col justify-start items-center bg-[#1c1a16]/80 rounded-md backdrop-blur-sm p-5">
                    <button className="text-white bg-yellow-500 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] bg-red/60 px-3 py-1 rounded-md backdrop-blur-sm mb-7" onClick={() => { setAbrirTrabalenguas(false) }} title="cerrar">Cerrar</button>
                    {/* Info */}
